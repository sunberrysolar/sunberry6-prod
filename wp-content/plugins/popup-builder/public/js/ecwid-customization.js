...
Ecwid.OnAPILoaded.add(function() {
  logDebug("Ecwid API chargée 9h12");
  ...
  // Correction globale pour intercepter les interactions
  document.addEventListener('click', function(e) {
    // Cibler spécifiquement les liens de code promo
    if (e.target && 
        (e.target.closest('.ec-cart-coupon__text .ec-link') || 
         e.target.closest('.ec-cart__coupon .ec-link'))) {
      
      // Tenter de déclencher l'ouverture du code promo de manière sécurisée
      try {
        const couponLinks = document.querySelectorAll('.ec-cart-coupon__text .ec-link, .ec-cart__coupon .ec-link');
        couponLinks.forEach(link => {
          // Forcer un événement de clic avec tous les événements
        ...
        console.error("Erreur lors de l'interaction avec le code promo:", error);
      }
    }
  }, true);
